# uv run 11-human-in-the-loop.py new_post\ai-workflows-vs-agents.txt
import base64
import sys
import os
...
def evaluate_article_draft(draft: str) -> Evaluation:
    ...
    if response.output_parsed is None:
        return Evaluation(needs_improvement=True, feedback="No evaluation could be generated.")
...
def generate_thumbnail(article: str) -> bytes:
    ...
    if not response.data or not hasattr(response.data[0], "b64_json") or response.data[0].b64_json is None:
