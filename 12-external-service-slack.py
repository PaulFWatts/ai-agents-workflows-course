# uv run 12-external-service-slack.py new_post\ai-workflows-vs-agents.txt
import base64
import sys
import os
...
def generate_thumbnail(article: str) -> bytes:
    ...
    if not response.data or not getattr(response.data[0], "b64_json", None):
        raise ValueError("No image data returned from OpenAI image generation API.")
    image_b64 = response.data[0].b64_json
    if image_b64 is None:
        raise ValueError("Image base64 data is None.")
