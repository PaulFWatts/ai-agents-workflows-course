# uv run 13-final-version.py new_post\ai-workflows-vs-agents.txt
import base64
import sys
import os
...
def evaluate_article_draft(draft: str) -> Evaluation:
    ...
    if response.output_parsed is None:
        # Provide a default Evaluation if parsing failed
        return Evaluation(needs_improvement=True, feedback="No evaluation could be parsed from the response.")
...
def generate_thumbnail(article: str) -> bytes:
    ...
    if not response.data or not getattr(response.data[0], "b64_json", None):
        raise ValueError("No image data returned from the image generation API.")

    b64_json = response.data[0].b64_json
    if b64_json is None:
        raise ValueError("Image data is missing (b64_json is None).")
...
def main():
    ...
    send_slack_notification("New blog post draft generated!")
